...
type RegistryService interface {
	...
	GetRegistry(ctx context.Context) (*regtypes.Registry, string, error) // returns registry, source, error

	// ListServers returns all servers in the registry
	ListServers(ctx context.Context) ([]regtypes.ServerMetadata, error)

	// GetServer returns a specific server by name
	GetServer(ctx context.Context, name string) (regtypes.ServerMetadata, error)
...
func (s *regSvc) GetRegistry(ctx context.Context) (*regtypes.Registry, string, error) {
...
func (s *regSvc) ListServers(ctx context.Context) ([]regtypes.ServerMetadata, error) {
	if err := s.refreshDataIfNeeded(ctx); err != nil {
		logger.Warnf("Failed to refresh data: %v", err)
	}
...
func (s *regSvc) GetServer(ctx context.Context, name string) (regtypes.ServerMetadata, error) {
	if err := s.refreshDataIfNeeded(ctx); err != nil {
		logger.Warnf("Failed to refresh data: %v", err)
	}
...
func (s *regSvc) getAllServersWithNames() []regtypes.ServerMetadata {
	servers := make([]regtypes.ServerMetadata, 0, len(s.registryData.Servers)+len(s.registryData.RemoteServers))
...
func (s *regSvc) getServerByNameWithName(name string) (regtypes.ServerMetadata, error) {
