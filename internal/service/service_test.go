...
func TestService_GetRegistry(t *testing.T) {
	...
	tests := []struct {
		...
		validateResult func(*testing.T, *regtypes.Registry)
	}{
		{
			name: "successful registry fetch",
			setupMocks: func(m *mocks.MockRegistryDataProvider) {
				m.EXPECT().GetRegistryData(gomock.Any()).Return(&regtypes.Registry{
					Version:     "1.0.0",
				...
				m.EXPECT().GetSource().Return("file:/path/to/regtypes.json").AnyTimes()
			},
			expectedSource: "file:/path/to/regtypes.json",
			validateResult: func(t *testing.T, r *regtypes.Registry) {
				t.Helper()
...
func TestService_ListServers(t *testing.T) {
	...
	tests := []struct {
		...
		setupMocks      func(*mocks.MockRegistryDataProvider)
		expectedCount   int
		validateServers func(*testing.T, []regtypes.ServerMetadata)
	}{
		{
			...
			expectedCount: 3,
			validateServers: func(t *testing.T, servers []regtypes.ServerMetadata) {
				t.Helper()
...
func TestService_GetServer(t *testing.T) {
	...
	tests := []struct {
		...
		setupMocks     func(*mocks.MockRegistryDataProvider)
		expectedError  string
		validateServer func(*testing.T, regtypes.ServerMetadata)
	}{
		{
			...
			validateServer: func(t *testing.T, s regtypes.ServerMetadata) {
				t.Helper()
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(reg *mocks.MockRegistryDataProvider, dep *mocks.MockDeploymentProvider) {
				reg.EXPECT().GetRegistryData(gomock.Any()).Return(&regtypes.Registry{
					Version: "1.0.0",
				}, nil)
